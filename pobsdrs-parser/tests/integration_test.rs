...
use pobsdrs_parser::{parser::ParserResult, Game, Parser};

#[test]
fn test_good_data() {
    let result = Parser::default()
        .load_from_file("tests/data/good-data.db")
        .unwrap();
    match result {
        ParserResult::WithoutError(_) => assert!(true),
        _ => assert!(false),
    }
    let games: Vec<Game> = result.into();
    assert_eq!(games[2].name, "Aedemphia".to_string());
}

#[test]
fn test_bad_data() {
    let result = Parser::default()
        .load_from_file("tests/data/bad-data.db")
        .unwrap();
    match result {
        ParserResult::WithError(games, lines) => {
            ...
            assert_eq!(lines[0], 5);
            assert_eq!(*lines.last().unwrap(), 16);
        }
        _ => assert!(false),
    }
}
